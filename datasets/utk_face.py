...
def _get_structured_utk_meta(metacsv):
    ...
    for line in csv_array[1:]:  # Exclude header
        data[line[0]] = {
            "cropped": line[0],
            "apparent_age": line[1],  # Adjust indices based on UTK-Face CSV format
            "real_age": line[2],
            "roi": get_roi_utk(line[3:7]),
        }
    return data

def structured_utk_data_wrapper(partition):

    """Wraps the metadata loading function for different partitions."""
    metacsv = 'utk_face/gt_avg_<part>.csv'
    ...
    return _get_structured_utk_meta(metacsv)

def _load_ages(metacsv, partition):

    """Loads age data for a specific partition."""
    global utk_ages
    if utk_ages is None or partition not in utk_ages or utk_ages[partition] is None:
        utk_ages[partition] = _get_structured_utk_meta(metacsv)
...
def get_age_from_utk(path, metacsv, partition):

    """Retrieves age and ROI data for a given image path."""
    _load_ages(metacsv, partition)
    try:
        cropped = utk_ages[partition][path]['cropped']
        age = utk_ages[partition][path]['apparent_age']
        roi = utk_ages[partition][path]['roi']
...
def _load_dataset(meta, csvmeta, imagesdir, partition, cropped=True):
    ...
    for item in tqdm(meta[1:]):  # Exclude header
        image_path = item[0]
        cropped_image_path, apparent_age, roi = get_age_from_utk(image_path, csvmeta, partition)
...
def _load_utk(csvmeta, imagesdir, partition, debug_max_num_samples=None):

    """Loads the UTK-Face dataset for a specific partition."""
    metapartition = get_metapartition_label(get_partition_label(partition))
    print("Directory partition:", metapartition)
    utk_partition_dir = imagesdir.replace('<part>', metapartition)
    utk_partition_csv = csvmeta.replace('<part>', metapartition)
    utk_partition_meta = _readcsv(utk_partition_csv, debug_max_num_samples)
    print("CSV {} read complete: {} samples".format(utk_partition_csv, len(utk_partition_meta)))
    return _load_dataset(utk_partition_meta, utk_partition_csv, utk_partition_dir, partition)

class UTKFaceAge:

    """Class for handling UTK-Face dataset loading and preprocessing."""
    def __init__(self,
                 partition='train',
                 imagesdir='utk_face/<part>',
                 csvmeta='utk_face/gt_avg_<part>.csv',
        ...
        cache_file_name = 'utk_face_age_{method}_{partition}{num_samples}.cache'.format(method=method, partition=partition, num_samples=num_samples)
        ...
        try:
            ...
            if partition == "all":
                ...
                for partition in ["train", "val", "test"]:
                    ...
                    self.data.extend(_load_utk(csvmeta, imagesdir, partition, debug_max_num_samples))
            else:
