...
def _get_structured_fgnet_meta(metacsv):
    ...
    for line in csv_array[1:]:  # Exclude header
        data[line[0]] = {
            "cropped": line[0],
            "apparent_age": line[1],  # Adjust indices based on FG-NET CSV format
            "real_age": line[2],
            "roi": get_roi_fgnet(line[3:7]),
        }
    return data

def structured_fgnet_data_wrapper(partition):

    """Wraps the metadata loading function for different partitions."""
    metacsv = 'fgnet/gt_avg_<part>.csv'
    ...
    return _get_structured_fgnet_meta(metacsv)

def _load_ages(metacsv, partition):

    """Loads age data for a specific partition."""
    global fgnet_ages
    if fgnet_ages is None or partition not in fgnet_ages or fgnet_ages[partition] is None:
        fgnet_ages[partition] = _get_structured_fgnet_meta(metacsv)
...
def get_age_from_fgnet(path, metacsv, partition):

    """Retrieves age and ROI data for a given image path."""
    _load_ages(metacsv, partition)
    try:
        cropped = fgnet_ages[partition][path]['cropped']
        age = fgnet_ages[partition][path]['apparent_age']
        roi = fgnet_ages[partition][path]['roi']
...
def _load_dataset(meta, csvmeta, imagesdir, partition, cropped=True):
    ...
    for item in tqdm(meta[1:]):  # Exclude header
        image_path = item[0]
        cropped_image_path, apparent_age, roi = get_age_from_fgnet(image_path, csvmeta, partition)
...
def _load_fgnet(csvmeta, imagesdir, partition, debug_max_num_samples=None):

    """Loads the FG-NET dataset for a specific partition."""
    metapartition = get_metapartition_label(get_partition_label(partition))
    print("Directory partition:", metapartition)
    fgnet_partition_dir = imagesdir.replace('<part>', metapartition)
    fgnet_partition_csv = csvmeta.replace('<part>', metapartition)
    fgnet_partition_meta = _readcsv(fgnet_partition_csv, debug_max_num_samples)
    print("CSV {} read complete: {} samples".format(fgnet_partition_csv, len(fgnet_partition_meta)))
    return _load_dataset(fgnet_partition_meta, fgnet_partition_csv, fgnet_partition_dir, partition)

class FGNetAge:

    """Class for handling FG-NET dataset loading and preprocessing."""
    def __init__(self,
                 partition='train',
                 imagesdir='fgnet/<part>',
                 csvmeta='fgnet/gt_avg_<part>.csv',
        ...
        cache_file_name = 'fgnet_age_{method}_{partition}{num_samples}.cache'.format(method=method, partition=partition, num_samples=num_samples)
        ...
        try:
            ...
            if partition == "all":
                ...
                for partition in ["train", "val", "test"]:
                    ...
                    self.data.extend(_load_fgnet(csvmeta, imagesdir, partition, debug_max_num_samples))
            else:
                print("Loading %s data from scratch" %